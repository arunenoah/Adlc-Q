...
const C = {
  ink: "#111", paper: "#fafaf7", line: "#111",
  accent: "#e8472c", ok: "#3a7a3a", warn: "#c98a2b",
  dim: "#888", soft: "#ececec", swarm: "#3a5a78",
};
...
// ---------- Tech Stack ----------
const TECH = {
  frontend: [{ id: "react", name: "React" }, { id: "vue", name: "Vue" }, { id: "next", name: "Next.js" }, { id: "svelte", name: "Svelte" }, { id: "blade", name: "Blade/Twig" }, { id: "none", name: "—" }],
  backend: [{ id: "node", name: "Node.js" }, { id: "python", name: "Python/FastAPI" }, { id: "go", name: "Go" }, { id: "rails", name: "Rails" }, { id: "php-laravel", name: "PHP/Laravel" }, { id: "php-yii2", name: "PHP/Yii2" }, { id: "php", name: "PHP" }],
  database: [{ id: "postgres", name: "Postgres" }, { id: "mongo", name: "MongoDB" }, { id: "mysql", name: "MySQL" }, { id: "supabase", name: "Supabase" }],
};
...
const findModelVariant = (variantId) => {
...
// ---------- Agents — each has a recommended model variant ----------
const AGENTS = [
...
const STDOUT_MANDATE = `CRITICAL OUTPUT REQUIREMENTS — read FIRST, obey ALWAYS:
...
const ROLE_PROMPTS_DRIFT = `Act as Tech Lead doing a drift audit.
${STDOUT_MANDATE}
...
// ---------- Pieces ----------
const Tag = ({ children, color = C.ink, bg = "transparent" }) => (
  <span style={{ ...mono, fontSize: 9, letterSpacing: "0.14em", textTransform: "uppercase", color, background: bg, border: `1px solid ${color}`, padding: "2px 6px" }}>
    {children}
  </span>
);
...
const Btn = ({ children, onClick, primary, disabled, small }) => (
  <button onClick={onClick} disabled={disabled}
    style={{
      ...
      color: disabled ? C.dim : C.paper, border: "none",
      padding: small ? "4px 8px" : "8px 14px",
      ...mono, fontSize: small ? 9 : 10,
...
const variantActive = (variantId, clis) => {
...
const migrateAgentModels = (m) => {
  if (!m) return defaultAgentModels();
  const out = { ...m };
...
// ---------- Changes Tab (renders git diff coloured) ----------
const STATUS_LABEL = { M: "modified", A: "added", D: "deleted", R: "renamed", U: "untracked", "?": "untracked" };
const STATUS_COLOR = { M: "#c98a2b", A: "#3a7a3a", D: "#c14a3c", R: "#3a5a78", U: "#3a7a3a", "?": "#3a7a3a" };

const DiffLine = ({ line }) => {
  const first = line[0] || "";
  let color = "#d6e2c7", bg = "transparent";
  ...
  return <div style={{ color, background: bg, padding: "0 6px", whiteSpace: "pre" }}>{line || " "}</div>;
};

const ChangesTab = ({ vm, setVm }) => {
  const toggle = (p) => setVm((prev) => prev ? { ...prev, expandedDiff: { ...prev.expandedDiff, [p]: !prev.expandedDiff?.[p] } } : prev);
  if (vm.changesLoading) return <div style={{ padding: 14, ...mono, fontSize: 11, color: C.dim }}>scanning git…</div>;
  if (vm.changesError) return <div style={{ padding: 14, ...mono, fontSize: 11, color: C.accent }}>error: {vm.changesError}</div>;
  if (!vm.changes) return <div style={{ padding: 14, ...mono, fontSize: 11, color: C.dim }}>click reload to scan</div>;
  ...
  return (
    <div>
      ...
      <div style={{ padding: "8px 14px", borderBottom: `1px solid ${C.line}`, background: C.soft, ...mono, fontSize: 10, color: C.dim }}>
        baseline: {vm.changes.baseline} {vm.sinceISO && <>· filtered to files modified ≥ {new Date(vm.sinceISO).toLocaleTimeString()}</>}
...
// ---------- Epic Review Gate (pause between steps) ----------
const EpicReviewModal = ({ gate }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
      <div style={{ background: C.paper, border: `1px solid ${C.line}`, padding: 18, width: 720, maxWidth: "92vw", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ ...mono, fontSize: 9, color: C.dim, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 8 }}>step output ({gate.taskKey})</div>
        ...
        <div style={{ ...mono, fontSize: 9, color: C.dim, letterSpacing: "0.12em", textTransform: "uppercase", margin: "10px 0 4px" }}>your note / answer (optional, fed to next step)</div>
        <textarea
          value={note}
...
export function DevOSShell({ initialProjects, initialClis, discoveredProjects }) {
  ...
  const mergedInitial = React.useMemo(() => {
    return initialProjects.map((p) => {
      const migratedModels = migrateAgentModels(p.agentModels);
